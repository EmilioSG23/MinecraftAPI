...
const isBiome = (item: any): item is Biomes => {
    return (
        ...
        Array.isArray(item.blocks) && item.blocks.every((b: any) => typeof b === "string") &&
        (item.structures === undefined || (Array.isArray(item.structures) && item.structures.every((s: any) => typeof s === "string")))
    );
};
...
const isMob = (item: any): item is Mobs => {
    return (
        ...
        Array.isArray(item.type) && item.type.every((t: any) => ["Animal", "Monster", "Aquatic", "Illager"].includes(t)) &&
        (item.speed === undefined || typeof item.speed === "number") &&
        Array.isArray(item.drop) &&
        item.drop.every(
            (dropItem: any) =>
                typeof dropItem.id === "string" &&
                typeof dropItem.min === "number" &&
                typeof dropItem.max === "number"
        )
    );
};
...
const isStructure = (item: any): item is Structures => {
    return (
        typeof item.id === "string" &&
        typeof item.name === "string" &&
        Array.isArray(item.biomes) && item.biomes.every((b: any) => typeof b === "string") &&
        (item.lootBox === undefined ||
            (Array.isArray(item.lootBox) &&
                item.lootBox.every(
                    (loot: any) =>
                        typeof loot.id === "string" && typeof loot.chance === "number"
                )))
    );
};

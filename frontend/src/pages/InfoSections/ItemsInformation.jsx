...
function ItemInformation({data, tooltip, onLoad}){
    return (
        <button className="flex relative group cursor-pointer w-[56px] h-[56px]" style = {{display: data.hidden ? "none" : "flex"}}
             onMouseEnter={() => {
                tooltip.setContent(
                     <div className="text-left">
                         <h3 className={getTierColor(data.tier)+" text-[16px]"}>{data.name}</h3>
                         <span className="text-[#545454]">{PREFIX_MC}{data.id}</span>

        ...
        <div className={`mc-container mx-auto max-w-7xl mt-7 flex flex-col items-center p-8 ${
          ...
          <div className='w-full flex flex-wrap overflow-y-scroll h-[624px] my-5 gap-y-10 justify-center'>
            {filteredDatas.map((data) => {
              return(
                <article key={data.id} className='flex flex-col gap-x-1 w-[32%] mx-1 bg-green-600 rounded-2xl border-3 border-green-900 overflow-hidden justify-center max-h-[360px]'
                  style = {{display: data.hidden ? "none" : "flex"}}>
                  <img src = {data.image} className='w-[384px] h-[200px]' onLoad={addImageLoaded} onError={addImageLoaded}/>
                  <h2 className = "text-center font-bold text-[26px] text-white border-t-3 border-green-900">{data.name}</h2>
                  <span className = "text-center -translate-y-2 text-gray-700">{`${PREFIX_MC}${data.id}`}</span>

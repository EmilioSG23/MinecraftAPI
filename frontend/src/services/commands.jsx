...
export function useCommands({setPanorama, setBlur, setDisplayMode}){
    ...
    return {inputCommand, setInputCommand, displayAuto, historyCommands, displayCommands, executeInputCommand, previousHistoryCommand, nextHistoryCommand, setDisplayCommands}
}
...
export function executeCommand(inputCommand, context){
    ...
    return (<p className="text-red-500">{"Unknown Command. Enter a new command and try again. Use /help to see all the commands."}</p>)
}
...
async function executeGet(args) {
    if (args.length !== 2){
        return <p className="text-red-500">Syntax Error: /get needs only two arguments. The correct syntax is: /get {"<type> <id>"}</p>;
    }
    ...
    return (status === 200 && (
            ...
            {Object.entries(data).map(([key, value]) => {
                return (<p key={key+"-"+value}><span className="underline">{key}:</span> {value.toString()}</p>)
            })}
            </>
            )) || (status === 404 && (<p className="text-red-500">Data not found. {data.message}.</p>)) || (status === 400 && (<p className="text-red-500">{data}</p>))
}

// - /key <type> <id> <key> Obtiene la información de esa key
async function executeKey(args){
    if (args.length !== 3){
        return <p className="text-red-500">Syntax Error: /key needs only three arguments. The correct syntax is: /key {"<type> <id> <key>"}</p>;
    }
    const {data, status} = await obtainDatasByURL(`${args.join("/")}`)
    ...
    return (status === 200 && (
        ...
        )) || (status === 404 && (<p className="text-red-500">This data doesn&apos;t have this key. {data.message}.</p>) || (status === 400 && (<p className="text-red-500">{data}</p>)))
}
...
async function executeCount(args){
    if (args.length !== 1){
        return <p className="text-red-500">Syntax Error: /count needs only one argument. The correct syntax is: /count {"<type>"}</p>;
    }

    const {data, status} = await obtainDatasByURL(`${args[0]}/count`)

    return (status === 200 && (
        <p>{`There are ${data} elements available for ${args[0]}`}</p>
    )) || ((status === 404 || status === 400) && (<p className="text-red-500">{`${args[0]} is not a valid data type. Check the available data types with /list.`}</p>))
}

// - /keys <type> Claves de tipo de dato
async function executeKeys(args){
    if (args.length !== 1){
        return <p className="text-red-500">Syntax Error: /keys needs only one argument. The correct syntax is: /keys {"<type>"}</p>;
    }
...
async function executeStatus(){
    ...
    return (
        status === 200 ? (<p>You are connected with the Minecraft API! Status: 200</p>) : 
...
function executeVersion(args){
    if (args.length !== 1){
        return <p className="text-red-500">{"Syntax Error: /version needs only one argument between <mc|api>. The correct syntax is: /version <type>"}</p>
    }
    if (args[0].toLowerCase() === "mc")
        return <p>{`Minecraft API has all game datas until version: ${MC_VERSION}`}</p>
    if (args[0].toLowerCase() === "api")
        return <p>{`Minecraft API version: ${API_VERSION}`}</p>
...
function executeBlur(args, setBlur){
    if (args.length !== 1){
        return <p className="text-red-500">{"Syntax Error: /setblur needs only one value between 0 and 10. The correct syntax is: /setblur <value>"}</p>
    }
...
function executeDisplay(args, setDisplayMode){
    if (args.length !== 1){
        return <p className="text-red-500">{"Syntax Error: /setdisplay needs only one argument between <random|select>. The correct syntax is: /setdisplay <mode>"}</p>
    }
    ...
    return <p className="text-red-500">{"Input Error: The input mode is not valid, make sure you choose between random or select."}</p>
}

// - /setpanorama <panorama>
function executePanorama(args, setPanorama){
    if (args.length !== 1){
        return <p className="text-red-500">{"Syntax Error: /setpanorama needs only one argument between 1 and 10. The correct syntax is: /setpanorama <panorama>"}</p>
    }
